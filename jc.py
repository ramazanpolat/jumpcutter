...
parser.add_argument('--url', type=str,
                    help="A youtube url to download and process. Make sure to cut out the 'https://www.' part.")
...
parser.add_argument('--silent_speed', type=float, default=99999,
                    help="The speed that silent frames should be played at. Enter nothing for just jumpcutting.")
parser.add_argument('--frame_margin', type=float, default=1,
                    help="Make this 0 for the jumpiest footage, around 1 for normal, 2 if you're having some sounds cut out unintentionally, and >3 if you want it very smooth.")
...
parser.add_argument('--frame_quality', type=int, default=2,
                    help="Quality of frames to be extracted from input video. 1 is highest, 31 is lowest, 2 is the default.")
args = parser.parse_args()
# choice = input(
#     "Jumpcutter made by carykh and ThatOneCalculator! Enter Y for youtube video, Q for quit, or anything else to choose a file\n\n").upper()
root = Tk()
root.wm_state("iconic")  # gets rid of the stupid window
# if choice == "Y":
#     try:
#         url_get = simpledialog.askstring("Input", "Enter URL", parent=root)
#         if "youtube.com" not in url_get:
#             quit()
#         if "https://" in url_get:
#             url_get = url_get.strip("https://")
#         if "www." in url_get:
#             url_get = url_get.strip("www.")
#         if "youtube" not in url_get:
#             quit()
#         print("Downloading ", url_get)
#     except:
#         quit()
# elif choice == "Q":
#     quit()
# else:
root.filename = filedialog.askopenfilename(initialdir="/Desktop", title="Select video file",
...
#         SAMPLE_RATE = 44100
#     SILENT_THRESHOLD = input(
#         "Enter silent threshold. The volume amount that frames' audio needs to surpass to be consider \"sounded\". It ranges from 0 (silence) to 1 (max volume). Leave blank for default.\n")
...
#         silent_speed = 99999
#     sounded_speed = input(
#         "Enter sounded speed. The speed that sounded (spoken) frames should be played at. Typically 1. Leave blank for default.\n")
...
# if choice == "Y":
#     INPUT_FILE = download_file(url_get)
# else:
...
assert INPUT_FILE is not None, "Make sure there's an input file. Please don't run this via the shell, run it in CMD with `py jumpcutter.py -h`"
